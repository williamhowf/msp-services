...
{
    public class RmqPersonalRewardConsumer : IDisposable //clement 20200816 MDT-1580
    {
        ...
        public RmqPersonalRewardConsumer(string identity, IConnection connection, RabbitMQConfig rmqSettings)
        {
            _identity = identity;
        ...
        public void Run(CancellationToken workerCancellationToken)
        {
            try
            {
                ...
                {
                    _timeStampUtil.InsertLastActivityLogTimestamp("RMQ-PersonalReward-Consumer"); //voonkeong 20201125 MDT-1757
        ...
        public class MessageReceiver : DefaultBasicConsumer, IDisposable
        {
            ...
            #region Validator
            public ResultObject ConsumerValidator(PersonalRewardConsumerDto jsonMsg, ResultObject resultObject)
            {
                if (string.IsNullOrWhiteSpace(jsonMsg.SecurityToken))
                {
                    resultObject.ReturnCode = 8100;
                    resultObject.ReturnMessage = "Security Token Is Required.";
                }
                else if (string.IsNullOrWhiteSpace(jsonMsg.Guid))
                {
                    resultObject.ReturnCode = 8101;
                    resultObject.ReturnMessage = "Guid Is Required.";
                }
                else if (string.IsNullOrWhiteSpace(jsonMsg.TransactionId))
                {
                    resultObject.ReturnCode = 8102;
                    resultObject.ReturnMessage = "Transaction Id Is Required.";
                }
                else if (string.IsNullOrWhiteSpace(jsonMsg.Status))
                {
                    resultObject.ReturnCode = 8103;
                    resultObject.ReturnMessage = "Status Is Not Valid.";
                }
                else if (!util.InterfaceOutMegopolyTransactionExists(jsonMsg.Guid, jsonMsg.TransactionId, _settings.Debug))
                {
                    resultObject.ReturnCode = 8106;
                    resultObject.ReturnMessage = "Record Is Not Found";
                }
                else
                {
                    if (jsonMsg.Status == "SUCCESS") // check jsonMsg.CreditAmount, jsonMsg.Rate
                    {
                        if (!jsonMsg.CreditAmount.HasValue)
                        {
                            resultObject.ReturnCode = 8104;
                            resultObject.ReturnMessage = "Credit Amount Value Is Missing.";
                        }
                        else if (!jsonMsg.Rate.HasValue)
                        {
                            resultObject.ReturnCode = 8105;
                            resultObject.ReturnMessage = "Rate Value Is Missing.";
                        }
                    }
                }
                return resultObject;
            ...
            public override void HandleBasicDeliver(string consumerTag, ulong deliveryTag, bool redelivered, string exchange, string routingKey, IBasicProperties properties, byte[] body)
            {
                ...
                PersonalRewardConsumerDto jsonMsg = null;
                TransactionEnquiry_PersonalRewardsMegopolyDto callbackobj = null;
                ...
                try
                {
                    ...
                    try
                    {
                        jsonMsg = DeserializeToObject<PersonalRewardConsumerDto>(msg);
                        if (jsonMsg == null)
                        {
                        ...
                        if (_settings.ValidateToken)
                        {
                            if (properties.Headers.ContainsKey("timestamp_in_ms") && !string.IsNullOrEmpty(jsonMsg.SecurityToken))
                            {
                                ...
                                if (!util.ValidateTokens(jsonMsg.SecurityToken, timestampLong, _settings))
                                {
                                    ...
                                    {
                                        callbackobj = new TransactionEnquiry_PersonalRewardsMegopolyDto
                                        {
                                            SecurityToken = "",
                    ...
                    if (validation)
                    {
                        ...
                        PersonalRewardsMegopolyTransactionUpdate TrxUpdate = new PersonalRewardsMegopolyTransactionUpdate(jsonMsg, util);

                        try
                        {
                            if (TrxUpdate.UpdateTransaction())
                            {
                            ...
                            else
                            {
                                ...
                                responseObject.ReturnMessage = "Update failed.";
